...
- (void)viewDidLoad
{
	...
	done = [[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStylePlain target:self action:@selector(choiceDone)];
    ...
    if (ASSETHELPER.selectdPhotos.count==0) {
        done.title = [NSString stringWithFormat:@"%@", @""];
    }
    else{
        done.title = [NSString stringWithFormat:@"%@", @"完成"];
    }
}
...
- (void)preview{
	...
	if ([preview.title isEqualToString:@"z"]) {
		[[NSNotificationCenter defaultCenter] postNotificationName:@"hidddpreview" object:nil];
		return;
	}
